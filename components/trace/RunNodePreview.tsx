...
import type { RunMeta, ChainNode } from '@/lib/types'
...
export default function RunNodePreview({ node, run, onBranch, isBranching }: {
  ...
  if (node.kind === 'context') {
    return (
      <div className="border border-amber-200 rounded-2xl p-6 bg-amber-50">
        ...
        <p className="font-mono text-sm text-zinc-900">{node.file}.md</p>
      </div>
    )
  }

  // agent / report / gate / branch / loop
  const items = run.agentOutputs.filter(o => o.nodeId === node.id)
  if (items.length === 0) {
    return (
      <div className="border border-zinc-200 rounded-2xl p-6 bg-zinc-50 text-sm text-zinc-500">
        <span className="font-bold text-zinc-900">{node.id}</span> is referenced by the chain but did not run in this execution.
      </div>
    )
  }
  ...
  return (
    <div className="border border-zinc-200 rounded-2xl overflow-hidden bg-white">
      {items.map((output, i) => {
        const stepIndex = run.agentOutputs.indexOf(output)
        return (
            ...
            <div className="bg-zinc-50 px-6 py-4 border-b border-zinc-200 flex items-center justify-between gap-4">
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-medium text-zinc-400 uppercase tracking-wider">
                  {output.model ? `${output.model} • ` : ''}{output.latencyMs}ms
                </span>
              </div>
              <div className="flex items-center gap-4">
                {stepIndex !== -1 && (
                  <button
                    onClick={() => onBranch(stepIndex)}
