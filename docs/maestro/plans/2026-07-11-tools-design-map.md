...
3. **Convention compliance spot-check** — run the intended default models against `loop-protocol`'s `## Changes` contract a few times; calibrate how loud the violation warning needs to be. RESOLVED 2026-07-19 (#19): "intended default models" isn't a single list this repo actually asserts (`.env.example` vs. `workspace/agents/*.md` disagree) — see `docs/adr/0003-derisk-scripts-target-the-apps-actual-model.md` for the decision to test the app's actual wired model instead, and #16/#19 for the result (5/5 compliant on `gemma-4-31b-it`). Slice 2's warning-loudness call should read that ADR before assuming multi-model coverage exists.
