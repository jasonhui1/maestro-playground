...
- [x] **0.3 convention-compliance spot-check** — `scripts/derisk/convention-check.ts`: hit 2–3 intended default models ~5× each with a `loop-protocol`-style "end with `## Changes`" instruction; count compliance. Calibrates how loud Slice 2's violation warning must be. *May slip to the start of Slice 2 without blocking Slice 1.* DONE 2026-07-19 (#19): tested the app's actual wired model only, not 2-3 — see `docs/adr/0003-derisk-scripts-target-the-apps-actual-model.md` for why, and #16/#19 for the 5/5-compliant result. Script supports `DERISK_PROVIDER=openrouter` if Slice 2 wants a second model's number before deciding warning loudness.
