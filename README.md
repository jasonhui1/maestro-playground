...
* **`POST /api/runs/:runId/nodes/:nodeId/chat`** — talk to an `agent` or `decider` node that already ran. Body: `{ message }`. The reply continues the node's own transcript (its system prompt, input, output and earlier turns; thought and tool turns are not replayed) and streams as SSE `token` events, ending `chat_done { message }` or `error`. Each turn is appended to the node's `conversation` in `meta.json` and to its log under `## Conversation`, after `## Output`; the run itself is unchanged. 400 for other node kinds, a node with no successful output, or a blank message; 404 for an unknown run or node; 409 while the run is `running`.
