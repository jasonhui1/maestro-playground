...
* **`GET /api/runs`** — list of `RunMeta` across all runs, with optional query filters (`chainName`, `status`, `keyword`, `entityType`+`slug`, `branchedFromRunId` for a run's forks).
